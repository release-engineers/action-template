...
if [[ $command == "build" ]]; then
  ...
  docker build --tag "action-template:${version}" --file Dockerfile .
  exit 0
fi
...
if [[ $command == "run" ]]; then
  ...
  version=$(poetry version --short)
  shift
  docker run --rm --publish 8000:8000 "action-template:${version}" "$@"
  exit 0
fi
...
if [[ $command == "publish-ghcr" ]]; then
  debug
  version=$(poetry version --short)
  image_tag_local="action-template:${version}"
